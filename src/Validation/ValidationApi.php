...
class ValidationApi
{
    ...
    public function validated($controller, $action)
    {
        ...
        $error_code = $config->get('apinote.error_code', -1);
        $field_error_code = $config->get('apinote.field_error_code', 'code');
        $field_error_message = $config->get('apinote.field_error_message', 'message');
